...
WORKDIR /Matchflix-Frontend

# We copy the .csproj of our app to root and 
# restore the dependencies of the project.
COPY /Matchflix-Frontend/Matchflix-Frontend.csproj .
RUN dotnet restore "Matchflix-Frontend.csproj"

# We proceed by copying all the contents in
# the main project folder to root and build it
COPY /Matchflix-Frontend .
RUN dotnet build "Matchflix-Frontend.csproj" -c Release -o /build

# Once we're done building, we'll publish the project
# to the publish folder 
FROM build-env AS publish
RUN dotnet publish "Matchflix-Frontend.csproj" -c Release -o /publish
